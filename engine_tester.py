...
def main():
    ...
    parser.add_argument('--fast', action='store_true', help='Use faster timeouts (may cause false failures for thinking engines)')
    ...
    base_timeouts = {
        ...
        'isready': 3.0,          # Increased for safety
        'newgame': 3.0,          # Increased for engines that reset state
        'first_move_movetime': 3.0,  # Increased for thinking engines
        'first_move_timecontrol': 5.0,  # Increased for time control analysis
        'multi_sequence_single': 3.0,   # Increased for positional analysis
        'graceful_quit': 3.0,    # Increased for cleanup
    }
    
    # Use faster timeouts if --fast is specified
    if args.fast:
        base_timeouts = {
            'uci_handshake': 2.0,
            'isready': 1.0,
            'newgame': 1.0,
            'first_move_movetime': 1.5,
            'first_move_timecontrol': 2.0,
            'multi_sequence_single': 1.5,
            'graceful_quit': 1.0,
        }
    ...
    json_path = args.json or f'results/engine_test_report_{timestamp}.json'
    md_path = args.md or f'results/engine_test_report_{timestamp}.md'

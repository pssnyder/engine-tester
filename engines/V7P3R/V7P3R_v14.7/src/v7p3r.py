#!/usr/bin/env python3
"""
V7P3R Chess Engine v14.7 - BLUNDER PREVENTION SYSTEM

CRITICAL ISSUE in V14.6 (and earlier):
- Blunder firewall code existed but was NEVER CALLED in search pipeline
- No move filtering before tactical ordering - all moves considered equally
- Result: Catastrophic blunders (hanging pieces, bad captures, exposed king)
- V14.6 vs V14.4 game showed queen/rook/bishop blunders on nearly every turn

V14.7 BLUNDER PREVENTION:
- PRE-FILTERING: Safety checks BEFORE tactical move ordering
- FOUR-PART FIREWALL: King safety, Queen safety, Valuable pieces, Capture validation
- INTEGRATION: Filter unsafe moves in _recursive_search before _order_moves_advanced
- PERFORMANCE: Bitboard-based checks, early rejection, minimal overhead (<15% NPS)

SAFETY FILTER PRIORITY:
1. King Safety (CRITICAL) - Reject moves exposing king to undefended attack
2. Queen Safety (HIGH) - Reject moves leaving queen hanging
3. Valuable Pieces (MEDIUM) - Reject moves leaving rooks/bishops/knights hanging
4. Capture Validation (MEDIUM) - Reject losing trades (bishop for pawn, etc)

ARCHITECTURE:
- Phase 1: Generate legal moves
- Phase 2: FILTER unsafe moves (NEW - V14.7)
- Phase 3: ORDER remaining safe moves tactically
- Phase 4: SEARCH ordered safe moves

VERSION LINEAGE:
- v14.7: Blunder prevention through pre-filtering (CRITICAL FIX)
- v14.6: Phase-based evaluation (NPS optimization but blunder-prone)
- v14.5: UCI fixes, time management fixes
- v14.4: REGRESSION - broken move ordering
- v14.3: Emergency time management fixes
...
class V7P3REngine:
    ...
    def _recursive_search(self, board: chess.Board, search_depth: int, alpha: float, beta: float, time_limit: float) -> Tuple[float, Optional[chess.Move]]:
        ...
        # V14.7: BLUNDER PREVENTION - Filter unsafe moves BEFORE ordering
        safe_moves = self._filter_unsafe_moves(board, legal_moves)
        
        # V14.7: Order the safe moves tactically
        ordered_moves = self._order_moves_advanced(board, safe_moves, search_depth, tt_move)

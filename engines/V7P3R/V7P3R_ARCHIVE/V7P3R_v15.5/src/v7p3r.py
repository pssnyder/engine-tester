...
- Material safety net (rejects moves losing >200 cp without compensation)
- Bishop pair awareness in material calculation
...
v15.5 Changes from v15.4:
- Fixed: Evaluation now PST-first with material safety net (not blend)
- Keep strong positional play, add material sanity check
- Material floor: Reject moves losing >200 cp material without +300 cp position

v15.4 Issues (REVERTED):
- Blended eval (70/30 PST/material) weakened positional strength
- Lost to v15.1, v14.1, v12.6, MaterialOpponent, PositionalOpponent
- Only 21.4% score in gauntlet
...
class V7P3REngine:
    ...
    def _evaluate_position(self, board: chess.Board) -> int:
        """
        Evaluate the current position using piece-square tables + material safety
        
        v15.5: PST-first with material safety net (not blended)
        - Primary: PST evaluation (positional strength)
        - Safety net: Reject if material loss > 200 cp without +300 cp compensation
        - Bishop pair bonus in material calculation
        ...
        # v15.5: Calculate material for safety net (not for blending)
        material_score = 0
        ...
        # Material safety net: If we're down >200 cp material, PST must show +300 cp advantage
        # This prevents catastrophic material losses while keeping strong positional play
        MATERIAL_THRESHOLD = 200  # Material deficit threshold
        COMPENSATION_REQUIRED = 300  # Positional compensation needed
        
        if board.turn == chess.WHITE:
            if material_score < -MATERIAL_THRESHOLD and pst_score < material_score + COMPENSATION_REQUIRED:
                # Material loss without sufficient positional compensation
                return material_score - 100  # Penalize heavily
        else:
            if material_score > MATERIAL_THRESHOLD and pst_score > material_score - COMPENSATION_REQUIRED:
                # Material loss without sufficient positional compensation
                return material_score + 100  # Penalize heavily
        
        # Otherwise use PST evaluation (our strength)

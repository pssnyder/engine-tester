#!/usr/bin/env python3
"""
V7P3R Chess Engine v15.7
A UCI-compatible chess engine based on PositionalOpponent core with MOVE FILTERING.

Version 15.7: BACK TO BASICS - Let PositionalOpponent's PST do its job!

THE CLEAN APPROACH:
- Pure PST evaluation (PositionalOpponent's proven strength)
- Smart MOVE FILTERING before search (don't even present bad moves)
- Boost good captures, filter material-losing moves
- NO complex evaluation hacks, NO blend ratios, NO penalties

WHY THIS WORKS:
- PositionalOpponent never had material problems
- Problem: V7P3R was evaluating bad moves as if they were good
- Solution: Don't let bad moves into the search at all!
- Let PST eval choose from ONLY safe/winning moves

MOVE FILTERING RULES:
1. Good captures (win/equal material): BOOSTED priority
2. Recaptures: ALWAYS included (even if equal trade)
3. Material-losing moves: FILTERED OUT (not evaluated)
4. Safe positional moves: Normal PST evaluation
...
- Opening book from V15.3

v15.7 Philosophy:
"Give the engine good moves to choose from, let PST pick the best position."
...
class OpeningBook:
    """Opening book handler with Polyglot format support"""
    
    def __init__(self):
        self.book_moves = {}  # Dict[zobrist_key] -> List[(move, weight)]
        self.use_book = True
        self.book_depth = 8  # Max ply to use book
        self.book_variety = 50  # % chance to pick non-best book move
        self.external_book_path = None
        self._load_embedded_book()
    
    def _load_embedded_book(self):
        """Load embedded opening repertoire (common good moves)"""
        # Key openings with strong moves
        ...
        embedded = {
            ...
            "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1": [
                ...
                ("g1f3", 80),   # King's knight
                ("c2c4", 80),   # English
                ("b1c3", 40),   # Uncommon but solid
            ],
            # After 1.e4
            "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1": [
                ...
                ("e7e6", 80),   # French
                ("c7c6", 70),   # Caro-Kann
                ("g8f6", 70),   # Alekhine/Scandinavian prep
        ...
        # Convert FEN to zobrist for faster lookup
        for fen, moves in embedded.items():
            board = chess.Board(fen)
            key = chess.polyglot.zobrist_hash(board)
            self.book_moves[key] = moves
    
    def load_polyglot_book(self, filepath: str) -> bool:
        """Load external Polyglot opening book (.bin format)"""
        ...
        try:
            with open(filepath, 'rb') as f:
                while True:
                    entry = f.read(16)  # Polyglot entry is 16 bytes
                    if len(entry) < 16:
                        break
                    
                    # Polyglot format: key(8), move(2), weight(2), learn(4)
                    key = struct.unpack('>Q', entry[0:8])[0]
                    move_int = struct.unpack('>H', entry[8:10])[0]
                    weight = struct.unpack('>H', entry[10:12])[0]
                    
                    # Decode Polyglot move format
                    from_file = (move_int >> 6) & 7
                    from_rank = (move_int >> 9) & 7
                    to_file = move_int & 7
                    to_rank = (move_int >> 3) & 7
                    promotion = (move_int >> 12) & 7
                    ...
                    move_uci = chess.Move(from_sq, to_sq, promotion=promo_piece).uci()
                    
                    if key not in self.book_moves:
                        self.book_moves[key] = []
                    self.book_moves[key].append((move_uci, weight))
            
            self.external_book_path = filepath
            return True
        except Exception as e:
            print(f"info string Error loading book: {e}")
            return False
    ...
    def get_book_move(self, board: chess.Board) -> Optional[chess.Move]:
        ...
        # Check ply limit
        if board.fullmove_number > (self.book_depth // 2 + 1):
            return None
        ...
        # Get available moves from book
        book_entries = self.book_moves[key]
        legal_moves = set(move.uci() for move in board.legal_moves)
        
        # Filter to legal moves only
        valid_entries = [(move_uci, weight) for move_uci, weight in book_entries 
                        if move_uci in legal_moves]
        
        if not valid_entries:
            return None
        
        # Apply variety: sometimes pick non-best move
        if random.randint(0, 100) < self.book_variety and len(valid_entries) > 1:
            # Weighted random selection
            total_weight = sum(weight for _, weight in valid_entries)
            rand_weight = random.randint(0, total_weight - 1)
            
            cumulative = 0
            for move_uci, weight in valid_entries:
                cumulative += weight
                if cumulative > rand_weight:
                    return chess.Move.from_uci(move_uci)
        
        # Pick best (highest weight) move
        best_move_uci = max(valid_entries, key=lambda x: x[1])[0]
...
class V7P3REngine:
    ...
    def _evaluate_position(self, board: chess.Board) -> int:
        ...
        # Return score from White's perspective (negamax search will handle perspective)
        return pst_score
    ...
    def _mvv_lva_score(self, board: chess.Board, move: chess.Move) -> int:
        ...
        # Use approximate piece values for MVV-LVA
    ...
    def _is_recapture(self, board: chess.Board, move: chess.Move) -> bool:
        """
        Check if move is a recapture (capturing on square where we just lost material)
        v15.7: Always allow recaptures, even if equal trade
        """
        # This is a simplified check - in a full implementation,
        # track the last capture square
        # For now, just return False (could enhance later)
        return False
    
    def _calculate_material_delta(self, board: chess.Board, move: chess.Move) -> int:
        """
        Calculate material change for a move (positive = gain, negative = loss)
        v15.7: Simple but effective material tracking
        
        Returns:
            Material delta in centipawns (+ve = gain, -ve = loss)
        """
        piece = board.piece_at(move.from_square)
        if not piece:
            return 0
        
        piece_value = self._get_approximate_piece_value(piece.piece_type)
        delta = 0
        ...
        if board.is_capture(move):
            ...
            if captured:
                delta += self._get_approximate_piece_value(captured.piece_type)
        
        # Material lost if piece hangs after move
        board.push(move)
        to_square = move.to_square
        
        # After the move, it's the OPPONENT's turn
        # Check if opponent can capture our piece
        opponent_color = board.turn  # This is the opponent's color after our move
        our_color = not opponent_color
        
        if board.is_attacked_by(opponent_color, to_square):
            # Our piece is attacked by opponent - check if we defend it
            if not board.is_attacked_by(our_color, to_square):
                # Piece hangs - we lose it!
                delta -= piece_value
    ...
    def _filter_and_order_moves(self, board: chess.Board, moves: List[chess.Move], ply: int, 
                     tt_move: Optional[chess.Move] = None) -> List[chess.Move]:
        """
        v15.7: FILTER bad moves, ORDER good moves
        
        THE KEY INSIGHT: Don't evaluate material-losing moves at all!
        
        FILTERING (removes from consideration):
        - Moves that lose material (calculated material loss)
        - Exception: Recaptures (even if equal trade)
        - Exception: Checks (might have tactical compensation)
        
        ORDERING (priority for remaining moves):
        1. TT move
        2. Checkmate threats
        3. Good captures (win material)
        4. Checks
        5. Equal captures/Recaptures
        6. Killer moves
        ...
        # STEP 1: Filter moves - remove material-losing moves
        for move in moves:
            # Always include checks (might have tactics)
            if board.gives_check(move):
                filtered_moves.append(move)
                continue
            
            # Always include recaptures (natural responses)
            if self._is_recapture(board, move):
                filtered_moves.append(move)
                continue
            
            # Calculate simple material balance for this move
            material_delta = self._calculate_material_delta(board, move)
            
            # FILTER: Remove moves that lose material (delta < -50cp)
            if material_delta < -50:
                continue  # Skip this move entirely!
            
            # Include all other moves (safe, winning, equal trades)
            filtered_moves.append(move)
        
        # If we filtered everything, fall back to all legal moves
        if not filtered_moves:
            filtered_moves = moves
        
        # STEP 2: Order the filtered moves
        scored_moves = []
        ...
        for move in filtered_moves:
            ...
            # Good captures (win material)
                ...
                elif material_delta >= -50:
                    # Equal or small loss (recapture)
            ...
            # KING MOVES: VERY LOW PRIORITY (preserve castling rights!)
            # v15.7: Engine was moving king too early, losing castling
            elif piece and piece.piece_type == chess.KING:
                # Only score highly if it's castling
                if abs(move.from_square - move.to_square) == 2:
                    score = 150000  # Castling is good!
                else:
                    score = -50000  # Regular king moves: LOW priority
    ...
    def _search(self, board: chess.Board, depth: int, alpha: float, beta: float, 
        ...
        # Generate, filter, and order moves
        legal_moves = list(board.legal_moves)
        if not legal_moves:
            return self._evaluate_position(board), None
            
        # v15.7: Filter material-losing moves BEFORE search

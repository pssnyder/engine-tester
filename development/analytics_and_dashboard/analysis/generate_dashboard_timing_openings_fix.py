#!/usr/bin/env python3
"""
Chess Analytics Dashboard Generator - Fixed Version for Timing and Openings
...
class ChessDashboardGenerator:
    ...
    def load_data(self):
        ...
        # Add dummy openings data for demonstration if not present
        if "player_stats" in self.data and not self.data["player_stats"].get("favorite_openings"):
    ...
    def create_opening_chart_script(self):
        ...
        if "player_stats" in self.data and "favorite_openings" in self.data["player_stats"]:
            ...
            if openings:
                ...
                labels = [f'"{name}"' for name, _ in top_openings]
                data = [count for _, count in top_openings]
        ...
        if os.path.exists(timing_image_path):
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="row mt-4">
                        <div class="col-12">
                            <h4>Time Usage Analysis</h4>
                            <canvas id="timeUsageChart" width="800" height="300"></canvas>
                        </div>
                    </div>
                </div>
            </div>
    ...
    def create_timing_chart_script(self):
        """Generate JavaScript for timing analysis chart."""
        script = ""
        
        # Create dummy time usage data
        # In a real app, you would extract this from your analysis results
        time_categories = ["0-5s", "5-10s", "10-15s", "15-30s", "30-60s", "60+s"]
        time_opening = [45, 30, 15, 7, 2, 1]
        time_middlegame = [35, 30, 20, 10, 3, 2]
        time_endgame = [55, 25, 10, 5, 3, 2]
        
        script = f"""
        (function() {{
            var timeCtx = document.getElementById('timeUsageChart');
            if (timeCtx) {{
                var timeUsageChart = new Chart(timeCtx, {{
                    type: 'bar',
                    data: {{
                        labels: {json.dumps(time_categories)},
                        datasets: [
                            {{
                                label: 'Opening',
                                data: {json.dumps(time_opening)},
                                backgroundColor: '#3498db'
                            }},
                            {{
                                label: 'Middlegame',
                                data: {json.dumps(time_middlegame)},
                                backgroundColor: '#2ecc71'
                            }},
                            {{
                                label: 'Endgame',
                                data: {json.dumps(time_endgame)},
                                backgroundColor: '#e74c3c'
                            }}
                        ]
                    }},
        ...
        <body>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {{
                    ...
                    // Timing chart
                    {self.create_timing_chart_script()}
